...
export default function Footer({ labels, locale }: { labels: Labels; locale: string }) {
  ...
  return (
    <footer className="footer-surface brand-shell site-footer-shell mt-10 border-t" role="contentinfo">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 py-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-4">
          <Link href={`/${locale}/privacy`} className="site-footer-link text-sm transition-colors hover:underline">
            {labels.privacy}
          </Link>
          <nav aria-label={labels.social} className="flex items-center gap-3">
            <Link
              ...
              aria-label="Facebook"
              className="site-footer-link text-sm transition-colors hover:underline"
              target="_blank"
