...
export default async function ServicesPage({
  ...
  const m = (await import(`@/locales/${locale}.json`)).default as {
    services: {
      ...
      process: {title: string; steps: {name: string; desc: string}[]};
      cta: string;
    };
  };
    ...
    <div className="py-10 space-y-10">
      ...
      <section>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          {m.services.process.steps.map((s: {name: string; desc: string}, i: number) => (
              ...
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <span
                    ...
                    className="inline-flex h-8 w-8 items-center justify-center rounded-md bg-primary/10 text-primary"
                  >
                    {i === 0 && <Compass size={18} />} {i === 1 && <Palette size={18} />}{" "}
                    {i === 2 && <Code2 size={18} />} {i === 3 && <Rocket size={18} />}
                  </span>
                  {s.name}
                </CardTitle>
              </CardHeader>
